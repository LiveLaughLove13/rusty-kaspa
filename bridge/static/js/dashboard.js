...
function applyTrendsViewMode(mode) {
  ...
  if (intro) {
    intro.innerHTML = isLong
      ...
      : 'Session: live charts (~2s refresh; cleared when this tab closes). Long range: Prometheus helpers and optional embed. Bridge status is in the <strong class="text-gray-400 font-medium">header</strong>; host details in <strong class="text-gray-400 font-medium">Bridge host</strong> below.';
  }
...
(function bootstrapFromCache() {
  ...
  if (!filter && icpuBoot && typeof icpuBoot === 'object') {
    const tr = document.createElement('tr');
    tr.className = 'border-b border-card/50 cursor-pointer';
