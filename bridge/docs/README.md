...
The bridge includes a built-in web dashboard at the configured `web_dashboard_port` (sample `config.yaml` uses `:3030`).

**Access:** Open `http://127.0.0.1:3030/` (or your configured port) in a web browser.

**Note:** The web dashboard is only started if `web_dashboard_port` is configured (non-empty). If no config file is used and no `--web-dashboard-port` is specified, the dashboard will not be available.
